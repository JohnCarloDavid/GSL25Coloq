...
if (isset($_GET['restore_id'])) {
    ...
    if ($deleted_order) {
        // Insert the deleted order back into tb_orders while keeping the original order_date
        $insert_sql = "INSERT INTO tb_orders (order_id, customer_name, product_name, size, quantity, order_date) 
                       VALUES (?, ?, ?, ?, ?, ?)";
        $insert_stmt = $conn->prepare($insert_sql);
        $insert_stmt->bind_param('isssis', 
            $deleted_order['order_id'], 
            $deleted_order['customer_name'], 
            $deleted_order['product_name'], 
            $deleted_order['size'], 
            $deleted_order['quantity'], 
            $deleted_order['order_date'] // Ensure original order_date is used
        );
        ...
        if ($insert_stmt->execute()) {
            ...
            // Redirect back to the same page after restoring the order
            header("Location: employee-recently-delete.php");
            exit;
        } else {
...
// Handle deletion of a single deleted order
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()) : ?>
                    <tr>
                        ...
                        <td>
                            <a href="employee-recently-delete.php?restore_id=<?= $row['order_id']; ?>" class="button" onclick="return confirm('Are you sure you want to restore this order?');">Restore</a>
                            <a href="employee-recently-delete.php?delete_id=<?= $row['order_id']; ?>" class="button" onclick="return confirm('Are you sure you want to delete this order?');">Delete</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
