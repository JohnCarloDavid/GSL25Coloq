...
if (isset($_GET['restore_id'])) {
    ...
    if ($deleted_order) {
        // Insert the deleted order back into tb_orders using its original order_date
        $insert_sql = "INSERT INTO tb_orders (order_id, customer_name, product_name, size, quantity, order_date) 
                       VALUES (?, ?, ?, ?, ?, ?)";

        $insert_stmt = $conn->prepare($insert_sql);
        $insert_stmt->bind_param('isssis', $deleted_order['order_id'], 
                                 $deleted_order['customer_name'], 
                                 $deleted_order['product_name'], 
                                 $deleted_order['size'], 
                                 $deleted_order['quantity'], 
                                 $deleted_order['order_date']); // Use the correct order_date
...
$sql = "
    SELECT d.*, COALESCE(i.price, 0) AS price 
                ...
                <tbody>
                    <?php while ($row = $result->fetch_assoc()) : ?>
                        <tr>
                            ...
                            <td>
                                <a href="recently-deleted.php?restore_id=<?= $row['order_id']; ?>" 
                                   class="button" 
